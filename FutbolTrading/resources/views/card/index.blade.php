...
<div class="container">
    ...
    <form action="{{ route('card.search') }}" method="GET" class="d-flex mb-4">
        <input type="text" name="query" class="form-control text-dark me-2" placeholder="{{ __('card.searchcards') }}" required>
        <button type="submit" class="btn btn-primary">{{ __('card.search') }}</button>
    </form>

    @if ($viewData['cards']->isEmpty())
        <p class="text-white">{{ __('card.noresults') }}</p>
    @else
        <div class="row">
                ...
                <div class="col-md-4">
                    <div class="card mb-4 bg-dark text-white">
                        <img src="{{ $card->getImage() ?? 'https://via.placeholder.com/150' }}" 
                            class="img-fluid rounded-top" alt="{{ $card->getName() }}" style="width: 100%; height: auto;">

                        <div class="card-body bg-dark">
                            <h5 class="card-title text-uppercase fw-bold">{{ $card->getName() }}</h5>
                            <p class="card-text">{{ __('card.descriptionindex') }} {{ $card->getDescription() }}</p>
                            <p class="card-text"><strong>{{ __('card.priceindex') }}</strong> ${{ number_format($card->getPrice(), 2) }}</p>
